...
void Opzione::setup_system ()
{
        ...
        triangulation.refine_global (refine_value);
	...
	cout<<"solution:\n";
             ...
             time-=time_step, --timestep_number)
        {
                ...
                */
                ...
                (Smax-par.K*exp(-par.r*(par.T-(timestep_number-1)*time_step)))+
                system_M2(1,2)*(Smax-par.K*exp(-par.r*(par.T-(timestep_number)*time_step)));
...
double Opzione::GetPrice() const
{
        double Price=0;
        /*
        double sol[dof_handler.n_dofs()];
        double grid[dof_handler.n_dofs()];
        double delta=(xmax-xmin)/(dof_handler.n_dofs()-1);
        ...
        gsl_spline *my_spline_ptr = gsl_spline_alloc (gsl_interp_cspline, dof_handler.n_dofs());
        gsl_spline_init (my_spline_ptr, grid, sol, dof_handler.n_dofs());
        
        double Price=gsl_spline_eval(my_spline_ptr, par.S0 , my_accel_ptr);
