...
template<int dim>
class Base_Fun: public Function<dim>
{
	
public:
	Base_Fun() = delete;
	Base_Fun(const Vector< Point<dim> >& grid, const Vector< Point<dim> >& int_grid, double tol):
	_grid(grid), _int_grid(int_grid), _tol(tol) {};
	Base_Fun(const vector< Point<dim> >& grid, const vector< Point<dim> >& int_grid, double tol):
	_grid(grid), _int_grid(int_grid), _tol(tol) {};
	
	virtual double value(unsigned int i, const Point<dim>& pt);
	
	
private:
	const vector< Point<dim> >& _grid;
	const vector< Point<dim> >& _int_grid;
	double _tol;
	
};

template<int dim>
double Base_Fun<dim>::value(unsigned int i,  const Point<dim>& pt) {
...
void Opzione<dim>::make_J_matrix() {
	...
	Base_Fun<dim> base(grid_points, integral_grid_points, 1e-10);
