...
private:
	...
	double Smin, Smax, xmin, xmax;
		...
		void integrale2_Levy2(Vector<double> &J, Vector<double> const &x);
...
//experimental,  need to use payoff in the right way,  not explicitely
template<int dim>
void Opzione<dim>::integrale2_Levy2(Vector<double> &J, Vector<double> const &x) {
	
	
	int n=J.size();
	int tot=dist_d+n+dist_u;
