...
public:
	...
	double lambda_meno_2;                                      // Parametro 4 Kou
        ...
        // Parametri della parte salto
        double C1;                               // Parametro 1 Merton                      
        double mu1;                              // Parametro 2 Merton
        double delta1;                           // Parametro 3 Merton
        
        
        // Parametri della parte salto
        double C2;                               // Parametro 1 Merton                      
        double mu2;                              // Parametro 2 Merton
        double delta2;                           // Parametro 3 Merton
...
private:
	...
	SparseMatrix<double>            ff_matrix;
        ...
        Quadrature_Hermite quad_x;
        std::vector<double> quad_nodes_x;
        std::vector<double> quad_weights_x;
        
        std::vector<Point<1> > quadrature_points_x;
        
        Quadrature_Hermite quad_y;
        std::vector<double> quad_nodes_y;
        std::vector<double> quad_weights_y;
...
void Opzione<dim>::Levy_integral_part1(){
	...
	/*
        ...
        */
        ...
        for (int i=0; i<quad_x.get_order(); ++i) {
                cout<<i<<"\n";
                alpha1+=(exp(quad_nodes_x[i])-1)*par.C1*quad_weights_x[i]/sqrt(2*pi*par.delta1*par.delta1);
                par.lambda1+=par.C1*quad_weights_x[i]/sqrt(2*pi*par.delta1*par.delta1);
        }
        
        for (int i=0; i<quad_y.get_order(); ++i) {
                cout<<i<<"\n";
                alpha2+=(exp(quad_nodes_y[i])-1)*par.C2*quad_weights_y[i]/sqrt(2*pi*par.delta2*par.delta2);
                par.lambda2+=par.C2*quad_weights_y[i]/sqrt(2*pi*par.delta2*par.delta2);
        }
        
        cout<<"alpha1 "<<alpha1<<" alpha2 "<<alpha2<<"\n";
        cout<<"lambda1 "<<par.lambda1<<" lambda2 "<<par.lambda2<<"\n";
...
//#pragma omp parallel for
                for (int it=0; it<J_x.size(); ++it) {
                        ...
                        // and some vectors
                        std::vector< Point<dim> > quad_points(quad_x.get_order());
                        std::vector<double> f_u(quad_x.get_order());
                        ...
                        for (int i=0; i<quad_x.get_order(); ++i) {
                                J_x(it)+=f_u[i]*par.C1*quad_weights_x[i]/sqrt(2*pi*par.delta1*par.delta1);
                        }
...
//#pragma omp parallel for
                for (int it=0; it<J_y.size(); ++it) {
                        ...
                        std::vector< Point<dim> > quad_points(quad_y.get_order());
                        std::vector<double> f_u(quad_y.get_order());
                        ...
                        // Integro dividendo fra parte sinistra e parte destra dell'integrale
                        for (int i=0; i<quad_y.get_order(); ++i) {
                                J_y(it)+=f_u[i]*par.C2*quad_weights_y[i]/sqrt(2*pi*par.delta2*par.delta2);
                        }
...
void Opzione<dim>::setup_system() {
	...
	system_rhs.reinit(dof_handler.n_dofs());
        ...
        quad_x=Quadrature_Hermite(static_cast<unsigned>(round((Bmax[0]-Bmin[0])/(dx1*5))), par.mu1, par.delta1);
        cout<<"Nodi "<<round((Bmax[0]-Bmin[0])/dx1)/5<<"\n";
        
        // Costruisco i vettori dei nodi e dei pesi
        quad_nodes_x=quad_x.get_nodes();
        quad_weights_x=quad_x.get_weights();
        
        quadrature_points_x=std::vector<Point<1> > (quad_x.get_order());
        
        for (unsigned i=0; i<quadrature_points_x.size(); ++i) {
                quadrature_points_x[i]=static_cast< Point<1> > (quad_nodes_x[i]);
        }
        
        // Costruisco punti e nodi di Hermite una volta per tutte (tanto non cambiano)
        quad_y=Quadrature_Hermite(static_cast<unsigned>(round((Bmax[1]-Bmin[1])/(dx2*5))), par.mu2, par.delta2);
        cout<<"Nodi "<<round((Bmax[1]-Bmin[1])/dx2)/5<<"\n";
        
        // Costruisco i vettori dei nodi e dei pesi
        quad_nodes_y=quad_y.get_nodes();
        quad_weights_y=quad_y.get_weights();
        
        quadrature_points_y=std::vector<Point<1> > (quad_y.get_order());
        
        for (unsigned i=0; i<quadrature_points_y.size(); ++i) {
                quadrature_points_y[i]=static_cast< Point<1> > (quad_nodes_y[i]);
        }
