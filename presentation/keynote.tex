...
Nelle Opzioni Americane: $$P(S,t)\geq \max(K-S_T,0)\qquad\forall t\in[0,T]$$
\pause
L'equazione diventa:
\begin{multline*}
\begin{cases}
...
\displaystyle
+\int_\mathbb{R}\left(P(t,S)-P(t,S)-S(e^y-1)\der{P}{S}(t,S)\right)\nu(dy)\alert{\leq} rP,\\
P(S,t)\alert{\geq} \max(K-S_T,0)
\end{cases}
\end{multline*}
\pause
Per risolvere questo problema si utilizza il PSOR:$$P_i^{(k+1)}=\max\left(K-S_i, P_i^{(k)}+\omega(z-P_i^{(k)})\right)$$
\end{frame}
...
\begin{equation*}
 M_1\mathbf{C}_h^k=M_2\mathbf{C}_h^{k+1}+M\mathbf{J}^{1,k}+M\mathbf{J}^{2,k}
\end{equation*}
\end{block}}
\end{frame}
...
\begin{block}{Vantaggi}
 \begin{itemize}
  \item Documentazione molto ampia e chiara, a cui si aggiunge la presenza di oltre 50 \emph{tutorial} programs che illustrano come usare la libreria per problemi pi\`u o meno tipici.
