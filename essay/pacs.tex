...
C(S,T)=max(S_T-K,0),\\
C(0,t)=0,\qquad\qquad\forall t\in[0,T],\\
...
P(S,T)=max(K-S_T,0),\\
P(0,t)=0,\qquad\qquad\forall t\in[0,T]\\
...
Le opzioni americane differiscono dalle europee poich\'e consentono di esercitare l'opzione non solo in $T$, bens\`i in qualsiasi istante di tempo dalla stipula del contratto alla sua scadenza. In particolare, nel caso monodimensionale, occorre aggiungere ai problemi \ref{callbs1d} e \ref{putbs1d} rispettivamente i seguenti vincoli: $$C(S,t)\geq max(S_t-K,0), \qquad\forall t\in[0,T],$$ e $$P(S,t)\geq max(K-S_t,0),\qquad\forall t\in[0,T].$$
