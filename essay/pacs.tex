...
\end{esempio}
Osserviamo quindi che il valore a scadenza, cio\`e il \emph{payoff}, dell'opzione dipende soltanto dal valore del sottostante. Definiamo quindi il \todo{il payoff non è n processo se T è fisso, in questo caso una v.a}\emph{payoff} come processo stocastico in funzione di $S$.
\begin{definition}
...
\item calcoliamo il valore atteso del \emph{payoff}, cio\`e $\mathbb{E}\left[\Phi(S_T)\right]$;
\item scontiamo il valore atteso del \emph{payoff} con il tasso di interesse $r$.
\end{enumerate}
Sia quindi $C:\mathbb{R}^+\times[0,T]\rightarrow\mathbb{R}^+$, $C=C(S(t),t)$ il processo stocastico che descrive il valore di un'opzione. In particolare, \todo{forse ci vuole il condizionamento} se consideriamo una \emph{call} europea,
...
\label{putbs1d}
\end{equation}
Come possiamo osservare, si tratta di equazioni paraboliche \emph{backward} con dato finale. Il prezzo dell'opzione al tempo $t=0$ sar\`a dato dalla soluzione C o P, valutate in $S_t=S_0$, ovvero il valore dell'azione oggi e in $t=0$.
...
\item \todo{cosi da sola tanto vale non metterla}c'\`e continuit\`a stocastica.
\end{enumerate}
\end{definition}
\begin{definition}
...
\begin{align*}
 \hat{\alpha}&=S\der{C}{S}(t,S)\int_\mathbb{R}(e^y-1)\nu(y)dy \\
 \hat{\lambda}&=C(t,S)\int_\mathbb{R}\nu(y)dy
\end{align*}
l'equazione diventa
\begin{equation*}
\der{C}{t}+\frac{\sigma^2}{2}S^2\dder{C}{S}+(r-\hat{\alpha})S\der{C}{S}-(r+\hat{\lambda})C+ \int_\mathbb{R}C(t,Se^y)\nu(y)dy=0.
\end{equation*}
Come nel caso logprice, $\hat{\lambda}=\lambda$, mentre $\hat{\alpha}$ va calcolato tramite una qualche formula di quadratura, esattamente come nel caso logprice. A questo punto introduciamo nell'integrale il cambio d variabile
\begin{equation*}
 z=Se^y
\end{equation*}
...
Scegliamo il troncamento del dominio in modo analogo al caso logprice, ossia definiamo $S_{min}$ ed $S_{max}$ come in \eqref{cut}. Allo stesso modo, definiamo il dominio per l'integrale come $[S_{min},S_{max}]$. In questo modo è possibile utilizzare la stessa griglia per il calcolo dell'integrale.
...
Moltiplicando l'equazione per una funzione test $\phi_i$, integrando sul dominio e inserendo $C_h$ al posto di $C$ otteniamo la discretizzazione della formulazione debole del problema \eqref{eq:pide1dlog}:
...
\end{multline*}
e integrando per parti il termine con la derivata seconda otteniamo, ricordando che i termini di bordo si annullano, 
...
Inroduciamo ora una discretizzazione temporale $\mathcal{T}=\left\{0= t_0\leq t_1\leq ... \leq t_N=T\right\}$ dell'intervallo $[0,T]$. Se consideriamo uno schema temporale del tipo Eulero Implicito, con la discretizzazione nel tempo l'equazione sopra diventa
...
+\left(r+\lambda+\frac{1}{dt}\right)\sum_{j=1}^{N-1}\gamma_j(t_n)\int_{\Omega_h}\phi_j(S)\phi_i(S)dS=\sum_{j=1}^{N-1}\frac{\gamma_j(t_{n+1})}{dt}\int_{\Omega_h}\phi_j(S)\phi_i(S)dS \\ \forall i \in \left\lbrace 1,\dots,N \right\rbrace \qquad \forall n \in \lbrace0,\dots,T-1\rbrace
\end{multline}

Definendo il vettore  $\bm{\gamma}^k$ come il vettore dei valori $\gamma_j(t_k)$ e le matrici seguenti
\begin{multline*}
...
\begin{equation*}
(M)_{ij}=\int_{\Omega_h}\phi_j(S)\phi_i(S)dS \quad M_2=\frac{1}{dt}M
\end{equation*}

il sistema d'equazionioni si può dunque scrivere in forma matriciale come 
...
\subsection{La parte integrale}
Al sistema \eqref{eq:PDEpriceMatrixes} va aggiunta la parte integrale. Anche in questo caso viene trattata esplicitamente e messa nel \emph{rhs}. Si tratta dunque di calcolare il seguente integrale

\begin{equation*}
 J(S)=\int_0^\infty \frac{C(t,z)}{z}\nu\left(\log\left(\frac{z}{S}\right) \right)dz
\end{equation*}
 che poi andrà scritto come elemento dello spazio $\mathcal{P}_h^1$. In pratica, si calcola l'integrale nei punti $S_i$ della griglia, ottenendo un vettore $\mathbf{J}$ dove $\mathbf{J}_i=J(S_i)$. La \eqref{eq:PDEpriceMatrixes} diventa dunque 
 
\begin{equation}
 \label{eq:PDEpriceMatrixeswithJ}
 M_1 \bm{\gamma}^n=M_2\bm{\gamma}^{n+1}+M\mathbf{J}^{n+1} \qquad \text{ per } n=T-1,\dots,0
\end{equation}
...
Notiamo che rispetto al metodo logprice non c'è più il problema dello shift sui nodi della griglia, ma il termine è comunque non locale perché per ogni vertice si integra su tutto il dominio.\\
Per il calcolo di tale termine, abbiamo deciso di adottare la seguente procedura: Per ogni cella della griglia, si calcola il valore della funzione $$\frac{C(t_k,z_l)}{z}$$ su opportuni nodi di quadratura. Poi, per ogni nodo $S_i$ della griglia si calcola il contributo di tale cella al valore di $\mathbf{J}^n_i$ valutandola contro la densità calcolata in $\log(\sfrac{z_l}{S_i})$
Riassumendo il termine $i$-esimo di $\mathbf{J}^k$ viene calcolato come
\begin{equation*}
 \mathbf{J}^n_i=\sum\limits_{Q_k\in \Omega_h}\left( \sum\limits_{z_l\in Q_k}\frac{C(t_n,z_l)}{z_l}\nu\left(\log\left(\frac{z_l}{S_i}\right)\right)\omega_l\right)
\end{equation*}
dove $Q_k$ sono le celle della griglia, e $z_l$ e $\omega_l$ sono i punti e i pesi di quadratura in quella cella. Notiamo che siccome la densità è calcolata in $\log(\sfrac{z_l}{S_i})$, non è più utile fare ricorso a formule di quadratura speciali per i modelli di Kou e Merton.
...
\begin{multline*}
 \der{C}{t}+(r-\hat{\alpha}^1)S_1\der{C}{S_1}+(r-\hat{\alpha}^2)S_2\der{C}{S_2}+\frac{\sigma^2_1}{2}S_1^2\dder{C}{S_1}+\frac{\sigma^2_2}{2}S_2^2\dder{C}{S_2}+\rho\sigma_1\sigma_2S_1S_2\dmix{C}{S_1}{S_2}\\-(r+\lambda^1+\lambda^2)C+\int_\mathbb{R}C(t,S_1e^y,S_2)\nu_1(y)dy+\int_\mathbb{R}C(t,S_1,S_2e^y)\nu_2(y)dy=0,
\end{multline*}
...
Come nel paragrafo precedente, trattiamo in modo separato parte differenziale e parte integrale. Il passaggio alla formulazione debole della parte differenziale è stata trattata per esempio in \cite{tao2009finite}, ed è analoga a quella monodimensionale. In definitiva, approssimando la funzione incognita $C$ come un elemento dello spazio
...
\begin{multline*}
 \sum_{j=0}^N\left[\der{C}{t}\gamma_j(t)\int\int_{\Omega_h}\phi_i\phi_j d\mathbf{S} +\gamma_j(t)\int\int_{\Omega_h}\phi_i(\mathbf{t}-\nabla D)\nabla \cdot \nabla\phi_j d\mathbf{S}\right.\\
 \left.-\gamma_j(t)\int\int_{\Omega_h}\nabla\phi_i D \nabla\phi_j d\mathbf{S}-(r-\lambda_1-\lambda_2)\gamma_j(t)\int\int_{\Omega_h}\phi_i\phi_j d\mathbf{S} \right]=0
\end{multline*}
...
\begin{multline*}
 \sum_{j=0}^N\left[\frac{\gamma_j(t_n)}{dt}\int\int_{\Omega_h}\phi_i\phi_j d\mathbf{S} -\gamma_j(t_n)\int\int_{\Omega_h}\phi_i(\mathbf{t}-\nabla D)\nabla \cdot \nabla\phi_j d\mathbf{S}\right.\\
 \left.+\gamma_j(t_n)\int\int_{\Omega_h}\nabla\phi_i D \nabla\phi_j d\mathbf{S}+(r-\lambda_1-\lambda_2)\gamma_j(t_n)\int\int_{\Omega_h}\phi_i\phi_j d\mathbf{S} \right]=\\
 =\sum_{j=0}^N\frac{\gamma_j(t_{n+1})}{dt}\int\int_{\Omega_h}\phi_i\phi_j d\mathbf{S}
\end{multline*}
...
Fin'ora non abbiamo menzionato le condizioni al contorno. Trattandosi sempre di condizioni di tipo \emph{dirichlet}, esse vengono imposte una
