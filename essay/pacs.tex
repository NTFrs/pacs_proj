...
\end{multline}
A questo punto, possiamo scrivere l'equazione in forma matriciale: $$M_1u^n=M_2u^{n+1}+MJ_1^{n+1}+MJ_2^{n+1},$$\todo{Scrivere esplicitamente le matrici di sistema} dove $M_1$ e $M_2$ sono le matrici di sistema date dalla formulazione debole e $M$ \`e la matrice di massa cosi definite:
\begin{equation*}
...
\\\\
In definitva, la libreria deal.ii si presenta come un prodotto solido e con una \emph{learning curve} molto morbida all'inizio, ma che necessita un po' pi\'u di sforzo se si vogliono utilizzare \emph{features} meno comuni.
