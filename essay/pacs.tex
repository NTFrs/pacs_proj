...
\end{equation*}
Come nel caso logprice, $\hat{\lambda}=\lambda$, mentre $\hat{\alpha}$ va calcolato tramite una qualche formula di quadratura. A questo punto introduciamo nell'integrale il cambio d variabile
\begin{equation*}
 z=Se^y
\end{equation*}
...
Definendo il vettore  $\underline{\gamma}^k$ come il vettore dei valori $\gamma_j(t_k)$ e le matrici seguenti
\begin{multline*}
...
\begin{equation}
 ...
 M_1 \underline{\gamma}^n=M_2\underline{\gamma}^{n+1} \qquad \text{ per } n=T-1,\dots,0
\end{equation}
...
\section{GitHub}
Durante lo sviluppo di questo progetto, abbiamo sempre utilizzato GitHub, un servizio di \emph{web hosting} per lo sviluppo di progetti software. Abbiamo quindi creato una \emph{repository} all'indirizzo \url{https://github.com/NTFrs/pacs_proj}. Commenti?!?!
\section{\textsf{Doxygen}}
Infine, tutto il codice \`e commentato con \textsf{Doxygen}. 
